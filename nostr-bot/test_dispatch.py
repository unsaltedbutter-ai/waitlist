...
async def test_login_registered_user(handler, mock_db):
    mock_db.get_user_by_npub.return_value = {"id": USER_ID, "debt_sats": 0, "onboarded_at": "2026-01-01"}
...
async def test_non_onboarded_member_gets_finish_setup(handler, mock_db):
    mock_db.get_user_by_npub.return_value = {"id": USER_ID, "debt_sats": 0, "onboarded_at": None}
...
@pytest.mark.asyncio
async def test_dispatch_passes_npub_to_commands(handler, mock_db, mock_commands):
    mock_db.get_user_by_npub.return_value = {"id": USER_ID, "debt_sats": 0, "onboarded_at": "2026-01-01"}
