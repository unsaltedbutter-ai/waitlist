...
class TestDriftCurve:
    """Post-click drift arc (hand relaxing)."""

    def test_starts_at_start(self) -> None:
        points = drift_curve((500, 500), distance=100, angle_deg=45)
        assert points[0] == (500, 500)

    def test_correct_point_count(self) -> None:
        points = drift_curve((0, 0), num_points=12)
        assert len(points) == 13  # num_points + 1 (inclusive endpoints)

    def test_endpoint_direction_up_right(self) -> None:
        """A 45-degree drift should move right and up (negative y in screen coords)."""
        points = drift_curve((500, 500), distance=100, angle_deg=45)
        end_x, end_y = points[-1]
        assert end_x > 500  # moved right
        assert end_y < 500  # moved up (screen coords: up = smaller y)

    def test_endpoint_distance_approximately_correct(self) -> None:
        start = (300, 400)
        dist = 150.0
        points = drift_curve(start, distance=dist, angle_deg=30, num_points=20)
        end_x, end_y = points[-1]
        actual_dist = math.hypot(end_x - start[0], end_y - start[1])
        # Bezier endpoint should be exactly at the computed end, but curvature
        # doesn't change the endpoint, only the path. Should be very close.
        assert abs(actual_dist - dist) < 1.0

    def test_zero_angle_moves_purely_right(self) -> None:
        """angle_deg=0 means pure horizontal drift (cos(0)=1, sin(0)=0)."""
        points = drift_curve((100, 200), distance=80, angle_deg=0)
        end_x, end_y = points[-1]
        assert abs(end_x - 180) < 1.0
        assert abs(end_y - 200) < 1.0

    def test_ninety_degrees_moves_purely_up(self) -> None:
        """angle_deg=90 means pure vertical drift upward in screen coords."""
        points = drift_curve((100, 200), distance=80, angle_deg=90)
        end_x, end_y = points[-1]
        assert abs(end_x - 100) < 1.0
        assert abs(end_y - 120) < 1.0  # 200 - 80
...
class TestJerkOffset:
    """Pre-move jerk (hand grabbing mouse)."""

    def test_starts_at_start(self) -> None:
        random.seed(42)
        points = jerk_offset((400, 300), distance=50)
        assert points[0] == (400, 300)

    def test_correct_point_count(self) -> None:
        random.seed(42)
        points = jerk_offset((0, 0), distance=50)
        assert len(points) == 7  # num_points=6 -> 7 inclusive

    def test_mostly_horizontal(self) -> None:
        """Jerk should be predominantly horizontal (small angle variance)."""
        random.seed(42)
        start = (500, 500)
        for _ in range(50):
            points = jerk_offset(start, distance=60)
            end_x, end_y = points[-1]
            dx = abs(end_x - start[0])
            dy = abs(end_y - start[1])
            # Horizontal displacement should dominate
            assert dx > dy, f"dx={dx}, dy={dy} not mostly horizontal"

    def test_distance_approximately_correct(self) -> None:
        random.seed(42)
        start = (300, 300)
        dist = 50.0
        points = jerk_offset(start, distance=dist)
        end_x, end_y = points[-1]
        actual_dist = math.hypot(end_x - start[0], end_y - start[1])
        assert abs(actual_dist - dist) < 5.0

    def test_direction_varies(self) -> None:
        """Over many calls, jerk should go both left and right."""
        random.seed(42)
        start = (500, 500)
        left_count = 0
        right_count = 0
        for _ in range(100):
            points = jerk_offset(start, distance=50)
            end_x = points[-1][0]
            if end_x < start[0]:
                left_count += 1
            else:
                right_count += 1
        assert left_count > 20
        assert right_count > 20
