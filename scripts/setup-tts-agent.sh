#!/usr/bin/env bash
# setup-tts-agent.sh — Install and configure the UnsaltedButter TTS Agent.
# Idempotent: safe to run after every git pull.
# macOS only (requires Chrome, pyautogui, pyobjc). Runs on Mac Studio.
#
# The TTS Agent depends on the agent package for browser/input code.
# This script installs agent/requirements.txt as well.
#
# Usage:
#   ./scripts/setup-tts-agent.sh              # install + configure
#   ./scripts/setup-tts-agent.sh --check      # verify env + deps only (no install)

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
COMPONENT_DIR="$PROJECT_ROOT/tts_agent"
VENV_DIR="$COMPONENT_DIR/venv"
ENV_DIR="$HOME/.unsaltedbutter"
SHARED_ENV_FILE="$ENV_DIR/shared.env"
ENV_FILE="$ENV_DIR/tts_agent.env"
PYTHON_VERSION="3.13"
...
check_only() {
    ...
    echo "=== TTS Agent Health Check ==="
    echo ""

    # OS
    if [ "$(uname -s)" != "Darwin" ]; then
        echo "OS:      $(uname -s) (WARNING: TTS Agent requires macOS)"
        ok=false
    else
    ...
    if [ -d "$VENV_DIR" ]; then
        ...
        if "$VENV_DIR/bin/pip" show httpx &>/dev/null; then
    ...
    # Check required vars
    local missing=()
    for var in API_BASE_URL AGENT_HMAC_SECRET; do
    ...
    echo ""
    echo "Note: TTS Agent requires Accessibility and Screen Recording permissions"
    ...
    if [ "$ok" = true ]; then
        ...
        echo "Status: NOT ready (run ./scripts/setup-tts-agent.sh to fix)"
    fi
}
...
main() {
    ...
    echo "=== UnsaltedButter TTS Agent Setup ==="
    echo ""

    # 1. OS check
    if [ "$(uname -s)" != "Darwin" ]; then
        echo "ERROR: TTS Agent requires macOS (Chrome + pyautogui + pyobjc)."
        exit 1
    fi
    echo "OS:     macOS ($(uname -m))"
    ...
    # 4. Install dependencies (agent deps first, then tts_agent deps)
    echo ""
    echo "Installing dependencies..."
    "$VENV_DIR/bin/pip" install --upgrade pip --quiet
    "$VENV_DIR/bin/pip" install -r "$PROJECT_ROOT/agent/requirements.txt" --quiet
    ...
    if [ ! -f "$ENV_FILE" ]; then
        cp "$PROJECT_ROOT/env-examples/tts_agent.env.example" "$ENV_FILE"
        chmod 600 "$ENV_FILE"
        echo "Created $ENV_FILE from env-examples/tts_agent.env.example (chmod 600)."
    ...
    if [ -x "$chrome" ]; then
        ...
        echo "  The TTS Agent needs Google Chrome installed to run."
    fi

    # 7. Smoke test
    echo ""
    echo "Running import smoke test..."
    if PYTHONPATH="$PROJECT_ROOT" "$VENV_DIR/bin/python" -c "from tts_agent.config import Config; from agent.browser import CHROME_PATH; import pyautogui; import httpx; print('All imports OK')" 2>&1; then
        :
    else
